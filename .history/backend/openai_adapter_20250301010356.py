...
class OpenAIAdapter(ModelAdapter):
    ...
    async def generate_text(
        ...
        try:
            # Use the provided model name or default to gpt-4-turbo-preview
    ...
    async def get_available_models(self) -> List[str]:
        ...
        try:
            ...
            logger.info(f"Got response from OpenAI: {len(models_data.get('data', []))} models")
            ...
            logger.info(f"Filtered to {len(chat_models)} chat models: {chat_models}")
            return chat_models
            
        except httpx.HTTPStatusError as e:
            logger.error(f"HTTP error occurred fetching OpenAI models: {e.response.status_code} - {e.response.text}")
            # Fall back to common models if we can't fetch the list
