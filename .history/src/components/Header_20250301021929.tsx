  ...
  return (
    ...
    >
      <Heading 
        size="md" 
        cursor="pointer" 
        onClick={() => location.pathname !== '/' && window.location.href = '/'}
      >
        {getPageTitle()}
        {activeConversation && !activeConversation.isActive && (
          <Badge ml={2} colorScheme="yellow">Archived</Badge>
        )}
      </Heading>
      ...
      <HStack spacing={3}>
        ...
        <Menu>
          ...
          <MenuList>
            <MenuItem icon={<Settings size={16} />} onClick={() => dispatch(openModal('settings'))}>
              Settings
            </MenuItem>
            <MenuItem icon={<Save size={16} />} onClick={() => dispatch(openModal('export'))}>
              Export Data
            </MenuItem>
            <MenuItem icon={<Upload size={16} />} onClick={() => dispatch(openModal('import'))}>
              Import Data
            </MenuItem>
            <MenuItem icon={<HelpCircle size={16} />} onClick={() => window.open('https://github.com/your-repo/ai-orchestrator/wiki', '_blank')}>
              Help
            </MenuItem>
          </MenuList>
        </Menu>
