...
import { updateMessage, addMessage } from '../store/slices/conversationSlice';
...
class WebSocketService {
  ...
  async connect(): Promise<void> {
    if (!this.isEnabled) {
      ...
      console.log('🔍 DEBUG - WebSocket connection skipped (disabled)');
      return;
    }
    ...
    try {
      ...
      console.log('🔍 DEBUG - Checking backend availability...');
      const isAvailable = await this.checkBackendAvailability();
      console.log("Backend availability check result:", isAvailable);
      if (!isAvailable) {
        console.log('🔍 DEBUG - Backend server is not available. WebSocket connection skipped.');
      ...
      console.log('🔍 DEBUG - Creating new WebSocket connection to:', this.url);
      ...
      this.socket.onopen = () => {
        ...
        // Completely disable connection notifications
        // No notifications for connections to reduce spam
      };
      ...
      this.socket.onclose = (event) => {
        console.log('🔍 DEBUG - WebSocket connection closed. Code:', event.code, 'Reason:', event.reason, 'Clean:', event.wasClean);
      ...
      this.socket.onerror = (error) => {
        ...
        if (this.reconnectAttempts === 0) {
          console.log('🔍 DEBUG - WebSocket connection error - backend may not be running');
        }
  ...
  private handleMessageChunk(data: any): void {
    ...
    // Enhanced logging for message streaming
  ...
  private handleMessageComplete(data: any): void {
    ...
    if (message) {
      // Enhanced logging for message completion
  ...
  private handleMultiAgentStarted(data: any): void {
    ...
    // Disable notifications to reduce spam
    // No notification for conversation started
  }

  private handleMultiAgentNextTurn(data: any): void {
    const { conversation_id, next_speaker_id, next_speaker_name, context } = data.data || {};
    
    console.log('🔍 DEBUG - Next turn:', next_speaker_name, '(', next_speaker_id, ')', 'context length:', context?.length || 0);
    ...
    const settings = store_state.settings;
    
    if (persona) {
      console.log(`🔍 DEBUG - Generating message from ${persona.name} using system prompt: ${persona.systemPrompt.substring(0, 50)}...`);
      
      // Create a placeholder message to show typing indicator
      const messageId = uuidv4();
      store.dispatch(addMessage({
        id: messageId,
        conversationId: conversation_id,
        senderId: next_speaker_id,
        senderType: 'agent',
        content: '...',
        timestamp: Date.now(),
        metadata: {
      ...
      this.send('generate_message', {
        ...
        message_delay: settings.messageDelay || 500  // Use configured delay or default to 500ms
      });
    } else {
  ...
  private handleMultiAgentNoNextSpeaker(data: any): void {
    ...
    console.log('🔍 DEBUG - No next speaker for conversation:', conversation_id, message);
    ...
    store.dispatch(addNotification({
      type: 'info',
      message: 'No next speaker available. Try sending a message to continue the conversation.',
      duration: 3000,
    }));
  }
